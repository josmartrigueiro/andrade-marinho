...
const logoVariants: Variants = {
  ...
  visible: {
    ...
    filter: "blur(0px)",
    transition: {
      duration: ANIMATION_TIMINGS.logoReveal,
  ...
  exit: {
    ...
    filter: "blur(12px)",
    transition: {
      duration: ANIMATION_TIMINGS.logoFadeOut,
      ease: ANIMATION_EASINGS.smooth,
    },
  },
};
...
const drawerVariants: Variants = {
  ...
  middle: {
    y: "0%",
    transition: {
      duration: ANIMATION_TIMINGS.drawerSlideDown,
...
function Logotype({ color, className = "" }: LogotypeProps) {
  return (
    <motion.svg
      width="400"
      height="149"
      ...
      className={className}
      role="img"
      aria-label="Logotipo Andrade Marinho"
      initial={{ fill: LOGO_COLORS.blue }}
      animate={{ fill: color }}
      transition={{
        duration: ANIMATION_TIMINGS.fillTransition,
        ease: ANIMATION_EASINGS.drawer,
      }}
    >
      <path d={LOGO_PATH} />
...
function ReducedMotionSplash({ bgColor }: { bgColor: string }) {
  return (
    <div
...
export function SplashScreen({
  ...
  const [drawerState, setDrawerState] = useState<"top" | "middle" | "bottom">(
    "top",
  );
  const [logoState, setLogoState] = useState<"hidden" | "visible" | "exit">(
    "visible",
  );
  const [logoColor, setLogoColor] = useState<string>(LOGO_COLORS.blue);
  const { isLoaded } = useImagePreload(preloadImages);

  const startTimeline = useCallback(() => {
    const drawerStartTime =
      (ANIMATION_TIMINGS.logoReveal + ANIMATION_TIMINGS.logoStay) * 1000;

    setTimeout(() => {
      setShowDrawer(true);
      setDrawerState("middle");
      setLogoColor(LOGO_COLORS.white);
    }, drawerStartTime);

    const logoExitTime =
      (ANIMATION_TIMINGS.logoReveal +
        ANIMATION_TIMINGS.logoStay +
        ANIMATION_TIMINGS.drawerSlideDown +
        ANIMATION_TIMINGS.logoWhiteStay) *
      1000;

    setTimeout(() => {
      setLogoState("exit");
      setDrawerState("bottom");
    }, logoExitTime);
  ...
  useEffect(() => {
    ...
    const runTimeline = () => {
      ...
      setTimeout(startTimeline, remainingMinTime);
    };

    if (isLoaded) {
      runTimeline();
    } else {
      timeoutId = setTimeout(runTimeline, maxDuration);
    }
  ...
  if (prefersReducedMotion) {
    return <ReducedMotionSplash bgColor={backgroundColorLight} />;
  }
  ...
  return (
    ...
    >
      <motion.div
        className={`absolute inset-0 ${backgroundColorLight}`}
        variants={backgroundVariants}
        initial="visible"
        animate={drawerState === "bottom" ? "hidden" : "visible"}
      />
      {showDrawer && (
        <motion.div
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <motion.div
          ...
          className="relative"
        >
          <Logotype color={logoColor} className="w-auto h-16 md:h-20 lg:h-24" />
        </motion.div>
      </div>
    </motion.div>
  );
